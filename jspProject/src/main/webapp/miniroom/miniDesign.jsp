...
<script>
	...
	function clickBackgroundRoom(room,num,image){
		frm = document.frm
	...
	<div id ="miniroom_design">
		...
		<section class ="miniroom_design_background_section">
			...
			<div class ="miniroom_design_background_div">
				<button onclick ="clickPrevIndex()"class ="index_button"><img class ="index_button_img" img src ="./img/Left.png"></button>
				<% for(int i = 0 ; i < backgroundList.size() ; i ++){
					ItemBean backgroundBean = backgroundList.get(i);
					String image = backgroundBean.getItem_path();
					int num = backgroundBean.getItem_num();
					String name = backgroundBean.getItem_name();
				%>
					<div id = "<%=num %>" onclick ="clickBackgroundRoom(this,'<%=num%>','<%=image %>') " class = "miniroom_design_background_room">
						<img id = "<%=num %>" class = "miniroom_design_background" alt = '<%=name %>' src='<%=image%>'">
					</div>
					<div class = "miniroom_design_background_separator"></div>
				<%} %>
				<button onclick ="clickNextIndex()" class ="index_button"><img class ="index_button_img" src ="./img/Right.png"></button>
			</div>
		</section>
